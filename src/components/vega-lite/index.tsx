...
export class VegaLite extends React.PureComponent<VegaLiteProps, VegaLiteState> {
  ...
  protected updateSpec() {
    ...
    try {
      ...
      const chartConfigObj = JSON.parse(localStorage.getItem('chartConfig'));
      const chartConfig = chartConfigObj.vegalite.en_us;

      const runtime = vega.parse(spec, chartConfig);
