...
async function uploadFile(name:string, file: string) {
  ...
  const CORS_PROXY = "https://observablecors.herokuapp.com/";
  const firebaseConfig = {
    apiKey: process.env.REACT_APP_apiKey,
    authDomain: "belo-voyager.firebaseapp.com",
    projectId: "belo-voyager",
    storageBucket: "belo-voyager.appspot.com",
...
// Raw string is the default if no format is provided
  uploadString(storageRef, file).then(() => {
    ...
    localStorage.setItem('chartURL', JSON.stringify({url:`${CORS_PROXY}https://firebasestorage.googleapis.com/v0/b/belo-voyager.appspot.com/o/${name}?alt=media&token=a64e0bf4-74cc-4405-9913-304331e792c9`})
    );
  });
...
export class DataSelectorBase extends React.PureComponent<DataSelectorProps, DataSelectorState> {
  ...
  private onDatasetChange(dataset: NamedData) {
    const urlEnd = dataset['url'].split('data/')[1];
    localStorage.setItem('chartURL', JSON.stringify({url:`https://unpkg.com/vega-datasets@1.25.0/data/${urlEnd}`})
    );
  ...
  private onFileChange(event: any) {
    ...
    reader.onload = async (lEvent: any) => {
      ...
      try {
        ...
        console.log('## DATA onFileChange:',values);
        console.log('## SET DATA !!!');
        await uploadFile(file.name, JSON.stringify(values));
      } catch (err) {
