...
export class PlotBase extends React.PureComponent<PlotProps, PlotState> {
  ...
  private get specWithFilter() {
    const {spec, filters, data, config} = this.props;
    const transform = (spec.transform || []).concat(toTransforms(filters));
    const dataSpec = JSON.parse(localStorage.getItem('chartURL'));
  ...
  private get specWithFilterCopy() {
    const {spec, filters, data, config} = this.props;
    const transform = (spec.transform || []).concat(toTransforms(filters));
    const dataSpec = JSON.parse(localStorage.getItem('chartURL'));
  ...
  private renderCopySpecButton() {
    ...
    const specStr = JSON.stringify(this.specWithFilterCopy, null, 2);

    const copyObj = '{\n\t"width": width,'+
                    specStr.slice(1, specStr.length) +
                    ',\n' +
